...
// return results for given start_time, caching ALL results and resolving nodes
...
void SchedulingExpression::cacheNodeResults(
    const Solver &solver, vector<map<double, vector<int> > > &partcap, int st) {
  ...
  // iterate over partitions; for each partition, query result for its part.var.
  for (int pi = 0; pi < this->partitions.size(); pi++) {
    ...
    // grab a mutable ref to the time slice for nodes avail. for given p and t
    vector<int> &nodesavail = partcap[p][this->_start_time];
    if (cnt > nodesavail.size()) {
...
vector<pair<double, int> > Choose::generate(
  ...
  TETRISCHED_DEBUG("The Choose Expression for " << jobName << "will be limited from time "
                   << curtime << " to " << sched_horizon << ".");
  TETRISCHED_DEBUG("The Choose Expression's start time is " << this->_start_time
                                                            << ".")
  ...
  // end at min(start_time + duration, sched_horizon) to avoid out-of-bounds in
  // partcap
  ...
  for (int pi = 0; pi < this->partitions.size(); pi++) {
    ...
    string vname = "P_" + jobName + "_time_" + std::to_string(this->_start_time) + "_" + std::to_string(p);
    Variable partvar(VAR_INT, pair<double, double>(0, this->_k), p, vname,
                     jobptr, this->_start_time, this->_duration, (double)pc0);
  ...
  cterms.push_back(
      pair<double, int>(-this->_k, I));            // complete demand constraint
...
vector<pair<double, int> > LinearChoose::generate(
  ...
  // end at the min(start_time + duration, sched_horizon) so that we do not go
  ...
  for (int pi = 0; pi < this->partitions.size(); pi++) {
    ...
    Variable partvar(VAR_INT,
                     pair<double, double>(
                         0, std::min((int)partcap[p][_start_time].size(), _k)),
    ...
    objf.push_back(pair<double, int>(_utility / (1.0 * _k) * utilFactor, vi));
  ...
  cterms.push_back(pair<double, int>(-_k, I));     // complete demand constraint
  m->addConstraint(Constraint(cterms, 0, OP_LE));  // add demand constraint
...
Allocation MaxExpression::getResults(
    const Solver &solver, vector<map<double, vector<int> > > &partcap,
    int start_time) {
  // iterate over cached indicator variables, return alloc for one enabled child
  for (const auto &p : this->cached_indvarmap) {
...
double MinExpression::upper_bound(vector<map<double, vector<int> > > &partcap) {
  ...
  for (double t = curtime; t < sched_horizon; t += 1) {
    ...
    for (auto c : chldarr) {
      ...
      ub4t = std::min(child_ub, ub4t);  // we will pick the smallest child util
    }
...
vector<pair<double, int> > MinExpression::generate(
  ...
  // TODO(atumanov): for each time slice, eval *this on union of nodes from all
  // p
  //                 and pick max util
  ...
  assert(ub >= 0);
  // U \in range [0; ub]
  Variable minUvar(VAR_FLOAT, pair<double, double>(0, ub), -1, "minU", jobptr,
                   0, 0,
  ...
  return vector<pair<double, int> >(1, pair<double, int>(1, vU));  // objf = U
}
...
Allocation MinExpression::getResults(
  ...
  // if the list of nodes is non-empty, assert that the start_times are the same
  Allocation allocres(vector<int>(), start_time, 0);  // r.v.
  for (auto c : this->chldarr) {
...
Allocation SumExpression::getResults(
  ...
  // iterate over cache, recursively aggregate results for all enabled children
  for (const auto &p : cached_indvarmap) {
...
void SumExpression::cacheNodeResults(
  ...
  for (const auto &p : this->indvarmap) {
    ...
    bool val = solver.getResult(vi);  // extract result for this decision var.
    this->cached_indvarmap[chld] = val;
  }
  marker = true;
}
...
vector<pair<double, int> > BarrierExpr::generate(
  ...
  return vector<pair<double, int> >(1,
                                    pair<double, int>(barrier * utilFactor, I));
}

// sole purpose of JobExpr is to pass the job pointer further down the expr tree
...
// Removes an element in child array, indicator variable array, and cache array
...
BarrierExpr::BarrierExpr(double _barrier, double _start_time, double _duration,
                         ExpressionPtr _chld) {
  barrier = _barrier;
  start_time = _start_time;
  duration = _duration;
...
void PreemptingExpression::populatePartitions(const vector<int> &node2part,
  ...
  SchedulingExpression::populatePartitions(node2part, curtime, sched_horizon);
  this->partition_nodes.clear();
  // Calcualte set of partitions
  for (int id : *_nodes) {
