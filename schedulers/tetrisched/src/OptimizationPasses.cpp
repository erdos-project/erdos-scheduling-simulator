...
namespace tetrisched
{
  ...
  DiscretizationSelectorOptimizationPass::DiscretizationSelectorOptimizationPass()
      : minDiscretization(1), maxDiscretization(5), maxOccupancyThreshold(0.8), OptimizationPass("DiscretizationSelectorOptimizationPass", OptimizationPassType::PRE_TRANSLATION_PASS) {}

  DiscretizationSelectorOptimizationPass::DiscretizationSelectorOptimizationPass(Time minDiscretization, Time maxDiscretization, float maxOccupancyThreshold) : minDiscretization(minDiscretization), maxDiscretization(maxDiscretization), maxOccupancyThreshold(maxOccupancyThreshold), OptimizationPass("DiscretizationSelectorOptimizationPass", OptimizationPassType::PRE_TRANSLATION_PASS) {}
      ...
      std::optional<std::string> debugFile)
  {
    ...
    while (!firstStack.empty())
    {
      ...
      else if ((currentExpression->getType() == ExpressionType::EXPR_CHOOSE || currentExpression->getType() == ExpressionType::EXPR_WINDOWED_CHOOSE) && currentExpression->getParents()[0]->getType() != ExpressionType::EXPR_MAX)
      {
        // TODO(Alind): Actual check should be that MAX has all children NCK for EXPR_CHOOSE
    ...
    {
      throw tetrisched::exceptions::RuntimeException("Inside Discretization Optimization pass: but no max over nck or independent ncks found!");
    }
    ...
    for (auto maxNckExpr : maxOverNckExprs)
    {
      ...
      occupancyRequestRanges.push_back(
          {occupancyRequestRange, numResources});
    }
    ...
    for (size_t i = 0; i < occupancyRequests.size(); i++)
    {
      ...
      // std::cout << "\t" << "[" << minOccupancyTime << "]" << "[DiscretizationSelectorOptimizationPass] "
      //                  << i + minOccupancyTime << ": " << occupancyRequests[i] << std::endl;
    }

    double autoMaxOccupancyThreshold = maxOccupancyThreshold * maxOccupancyVal;
    // std::cout << "** [DiscretizationSelectorOptimizationPass] Max Discretization Value" << maxOccupancyVal << " Threshold Decided is: " << autoMaxOccupancyThreshold << " threshold val: " << maxOccupancyThreshold << std::endl;
    ...
    auto linearDecayFunction = [this, autoMaxOccupancyThreshold](double curentOccupancy)
    {
    ...
    //             << discretizationTimeRange.first << " - " << discretizationTimeRange.second << " : " << granularity << " Occuapncy: " << occupancyRequests[discretizationTimeRange.first - minOccupancyTime] << std::endl;
    // }
  ...
  OptimizationPassRunner::OptimizationPassRunner(bool debug, bool enableDynamicDiscretization, Time minDiscretization, Time maxDiscretization, float maxOccupancyThreshold) : debug(debug), enableDynamicDiscretization(enableDynamicDiscretization) {
