                           ...
                           vector<map<double,vector<int> > > partcaps)
{
    ...
    int root_var_idx = mptr->addVariable(
            Variable(VAR_BOOL, pair<double,double>(0, 1), -1, "rootI", nullptr,
...
IloExpr CPLEXSolver::translateExpression(const vector<pair<double,int> > &terms,
...
IloRange CPLEXSolver::translateConstraint(const Constraint &mycon, const IloNumVarArray &vars)
{
    IloRange range;
...
bool CPLEXSolver::translateModel()
{
    ...
    for (const auto &var: mptr->getVariables()) {
        ...
        case VAR_INT:
        {
            IloIntVar cplexv(env, var.range.first, var.range.second, var.name.c_str());
            vars.add(cplexv);
            break;
        }
        case VAR_FLOAT:
        {
            IloNumVar cplexv(env, var.range.first, var.range.second, ILOFLOAT, var.name.c_str());
            vars.add(cplexv);
            break;
        }
        }
//        IloNumVar::Type t = int2exttype[var.vartype];
//        IloNumVar cplexv(env, var.range.first, var.range.second, t, var.name.c_str());
//        env.out() << "VAR: " <<cplexv<<endl;
    ...
    IloExpr objfexpr = translateExpression(mptr->getObjective().objfexpr, vars);
    if (mptr->getObjective().maximize) {
...
void CPLEXSolver::solve(double timeLimit)
{
    ...
    cplex.use(timeLimitCallback(env, cplex, IloFalse, cplex.getCplexTime(), timeLimit, 5.0));
    ...
    if (true) {
        ...
        cout << endl;
        //cplex.addMIPStart(this->vars, initvals, IloCplex::MIPStartCheckFeas);
        cplex.addMIPStart(this->vars, initvals, IloCplex::MIPStartAuto);
    }
...
// get specific results of interest: which jobs consume how much from which partitions
...
map<JobPtr, Allocation> CPLEXSolver::getResults()
{
    ...
    IloNumArray vals(env);
    // debugging
    //cout << "[solver]: variable array sizes: " << this->vars.getSize() << " " << mptr->getVariables().size();
...
double CPLEXSolver::getResult(int vidx) const {
    const vector<Variable> &internal_vars = mptr->getVariables(); // internal model
    assert(this->vars.getSize() == internal_vars.size());   // solver model vars
    assert(0 <= vidx && vidx < internal_vars.size());
    const Variable &v = internal_vars[vidx];
    switch(v.vartype) {
    case VAR_INT:
    {
