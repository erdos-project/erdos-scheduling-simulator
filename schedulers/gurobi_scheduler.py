...
class GurobiScheduler(ILPScheduler):
    def schedule(self, needs_gpu: List[bool], release_times: List[int],
        ...
        times = [s.addVar(vtype=GRB.INTEGER  ,name =f't{i}')
                 for i in range(0, num_tasks)]  # Time when execution starts
        costs = [s.addVar(vtype=GRB.INTEGER  ,name =f'c{i}')for i in range(0, num_tasks)]  # Costs of gap
        placements = [s.addVar(vtype=GRB.INTEGER  ,name =f'p{i}')
        ...
        for row_i in range(len(dependency_matrix)):
            for col_j in range(len(dependency_matrix[0])):
                disjoint = [
                    times[row_i] + expected_runtimes[row_i] <= times[col_j] - 1,
                ...
                if row_i != col_j:
                    alpha = s.addVar(vtype=GRB.BINARY  ,name =f'alpha{row_i}_{col_j}')
                    beta = s.addVar(vtype=GRB.INTEGER  ,name =f'beta{row_i}_{col_j}')
                    decision_vars.append((alpha,beta))
                    s.addConstr(
                         - alpha * M - beta * M + 1 <= placements[row_i] - placements[col_j]
                    )  # pi > pj
                    s.addConstr(
                         alpha * M + (1 - beta) * M -1 >= placements[row_i] - placements[col_j]
                    )  # pj > pi 
                    s.addConstr(
                         times[col_j] - times[row_i] <=  1 + expected_runtimes[row_i] + (1-alpha) * M + beta * M
                    )  # tj - ti <= 1 + ei
                    s.addConstr(
                         times[row_i] - times[col_j] <=  1 + expected_runtimes[col_j] + (1 - alpha) * M + (1-beta) * M
                    )  # ti - tj <= 1 + ej
