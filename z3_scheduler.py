from z3 import *
import time
def scheduler(requires_gpu, release_times, deadlines, expected_execution, dependency_matrix, pinned, num_tasks: int, GPUS: int, CPUS: int):
  times = [Int (f't{i}') for i in range(0,num_tasks)] #Time when execution starts
  placements = [Int(f'p{i}') for i in range (0,num_tasks)] # placement on CPU or GPU
  s = Solver()
  ...
  for row_i in range(len(dependency_matrix)):
    for col_j in range(len(dependency_matrix[0])):
      ...
      if dependency_matrix[row_i][col_j]:
        s.add(disjoint[0]) # dependent jobs need to finish before the next one
      if row_i != col_j:
        s.add(Implies(placements[row_i] == placements[col_j], Or(disjoint) )) # cannot overlap if on the same hardware
  ...
  if schedulable != unsat:
    return [s.model()[p] for p in placements], [s.model()[t] for t in times]
  return None
