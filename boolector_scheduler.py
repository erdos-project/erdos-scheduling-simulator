...
def scheduler(requires_gpu: bool,
              release_times,
              deadlines,
              expected_execution,
              dependency_matrix,
...
def run(MULTIPLIER, expected_runtime=5):
    ...
    deadlines = [HORIZON
                 for _ in range(0, TASKS)]  # deadline when task must finish
    expected_execution = [expected_runtime for _ in range(0, TASKS)
                          ]  # expected runtime :: 20 is best schedulable
