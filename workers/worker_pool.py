...
class WorkerPool:
    ...
    def __init__(self, num_cpus: int, num_gpus: int):
        ...
        self.workers_no_gpu = [
            Worker(i, num_gpus=0) for i in range(num_gpus, num_gpus + num_cpus)
        ]
    ...
    def get_running_tasks(self):
        return [(w.unique_id, w.current_task)
                for w in self.workers_gpu if w.current_task
                ] + [(w.unique_id, w.current_task)
                     for w in self.workers_no_gpu if w.current_task]

    def get_worker(self, i):
        assert i < self.count and i >= 0, "No such worker {i}"
        return self.workers_gpu[
            i] if i < self.num_gpus else self.workers_no_gpu[i - self.num_gpus]
