...
POISSON_ARRIVAL_RATES=(0.055 0.05) # Tune this
DAG_AWARENESS=(1) # False True
TASK_CPU_DIVISOR=25

WORKER_CONFIG=alibaba_cluster_20_slots

DYNAMIC_DISCRETIZATION=1
MAX_OCCUPANCY_THRESHOLD=0.7
MAX_TIME_DISCRETIZATION=5
...
--scheduler_max_time_discretization=${MAX_TIME_DISCRETIZATION}
--scheduler_max_occupancy_threshold=${MAX_OCCUPANCY_THRESHOLD}
"
        fi
                ...
                for MAX_DEADLINE_VARIANCE in ${MAX_DEADLINE_VARIANCES[@]}; do
                    for SCHEDULER_TIME_DISCRETIZATION in ${SCHEDULER_TIME_DISCRETIZATIONS[@]}; do
                        for DAG_AWARE in ${DAG_AWARENESS[@]}; do
                            ...
                            if [[ ${DYNAMIC_DISCRETIZATION} == 1 ]]; then
                                LOG_BASE+="_dynamic_max_occupancy_threshold_${MAX_OCCUPANCY_THRESHOLD}"
                            fi
