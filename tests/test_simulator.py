...
def test_event_queue_one():
    ...
    event_queue.add_event(Event(event_type=EventType.SCHEDULER_START,
                                time=1.0))
    event_queue.add_event(Event(event_type=EventType.SCHEDULER_FINISHED,
                                time=2.0))
...
def test_event_queue_two():
    """ Test that an EventQueue properly orders the given events. """
    event_queue = EventQueue()
    event_queue.add_event(Event(event_type=EventType.TASK_RELEASE,
                                time=3.0,
...
def test_simulator_handle_event():
    ...
    return_value = simulator._Simulator__handle_event(
            event=Event(event_type=EventType.SIMULATOR_END, time=1.0),
            task_graph=None)
